    ...
    def NFP(self, i, time):
        ...
        if time == (NbIterations - 1):
            ...
            if i == (Nb - 1): #Boundary condition, if we are at the "last" cell during a time iteration
                ...
                self.H = NbHCell*(self.HydroFraction[time + 1][i] - self.HydroFraction[time][i])*1.024E-18 #Energy difference in J for electrons 20eV and 13.6eV
                self.T[time + 1][i] = (2*self.dt*(self.H - self.psiH) + self.T[time][i]*(3*self.rho*(1-self.HydroFraction[time + 1][i])*kb))/((3*self.rho*(1-self.HydroFraction[time + 1][i])*kb) + self.dt*2*(3/2*self.rho*(1-self.HydroFraction[time + 1][i])*kb*(self.HydroFraction[time + 1][i]-self.HydroFraction[time][i]))/self.dt)
                ...
                self.H = NbHCell*(self.HydroFraction[time + 1][i] - self.HydroFraction[time][i])*1.024E-18 
                self.T[time + 1][i] = (2*self.dt*(self.H - self.psiH)+self.T[time][i]*(3*self.rho*(1-self.HydroFraction[time + 1][i])*kb))/((3*self.rho*(1-self.HydroFraction[time + 1][i])*kb) + self.dt*2*(3/2*self.rho*(1-self.HydroFraction[time + 1][i])*kb*(self.HydroFraction[time + 1][i]-self.HydroFraction[time][i]))/self.dt)
...
class Chemistry():
    ...
    def Get_Rates(self):
        return self.alphaAH(), self.alphaBH(), self.betaH(), self.psiH()
    ...
    def psiH(self):
        res  = 7.5E-19/(1 + pow(self.T/1E5 , 0.5))
...
if c*dt/dx < 1:
    ...
    StelObj1 = StellarSources(0, T[0]) #Without chemistry
    StelObj1.SelectSource(2)
    Y1       = StelObj1.Get_Data()[0] #0 for N and 1 for fraction
   
    #StelObj2 = StellarSources(1, T[0]) #With chemistry
    #StelObj2.SelectSource(2)
